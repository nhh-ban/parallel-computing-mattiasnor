...
#Rewritten using parallel computing
...
# Modify the loop for parallel processing using foreach from the doParallel package
results <- 
  foreach(
    i = 1:nrow(df), 
    .combine = 'rbind', 
    .packages = c('magrittr', 'tidyverse', 'tweedie')
    ) %dopar% {
  tibble(
    N = df$N[i], 
    M = df$M[i], 
...
# Convert the results back to the df dataframe
df$share_reject <- unlist(results$share_reject)

# Close the cluster after parallel processing
stopCluster(cl)
