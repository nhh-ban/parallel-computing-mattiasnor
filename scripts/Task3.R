...
MTweedieTests <- function(N, M, sig) {
  ...
  # Use foreach to parallelize the replicate function
  p_values <- foreach(j = 1:M, .combine = c, .packages = c('tweedie', 'stats')) %dopar% {
    simTweedieTest(N)
  }
  ...
  # Calculate the share of p-values below the significance threshold
  sum(p_values < sig) / M
}
